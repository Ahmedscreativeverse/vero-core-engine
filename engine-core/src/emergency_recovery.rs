...
pub fn request(env: &Env, requester: &Address, token_addr: &Address, dest: &Address, amount: i128) {
    crate::non_reentrant!(env);
    assert_closed(env);

    requester.require_auth();
    require_admin(env, requester);
    validate_address(env, token_addr);
...
fn execute_recovery(env: &Env, dest: &Address) {
    ...
    token::Client::new(env, &token_addr).transfer(&env.current_contract_address(), dest, &amount);
    publish_event(
        env,
...
mod tests {
    ...
    fn init_rejects_zero_threshold() {
        ...
        env.as_contract(&contract_id, || init(&env, vec![&env, admin], 0));
    }
    ...
    fn init_rejects_threshold_exceeding_admin_count() {
        ...
        let contract_id = env.register_contract(None, TestContract);
        let admin = Address::generate(&env);
        env.as_contract(&contract_id, || init(&env, vec![&env, admin], 2));
    }
