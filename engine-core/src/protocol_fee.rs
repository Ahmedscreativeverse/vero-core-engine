...
use soroban_sdk::{
    contracterror, panic_with_error, symbol_short, token, Address, BytesN, Env, String, Symbol,
};
...
fn validate_address(env: &Env, addr: &Address) {
    if addr.to_string().is_empty() || addr.to_string() == String::from_str(env, ZERO_ADDRESS) {
